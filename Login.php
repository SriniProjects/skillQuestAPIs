...
$fcm=$_GET['fcm'];
	...
	$q_login="SELECT * FROM `users` WHERE `email` LIKE '$email' AND `password` LIKE '$pwd'";
	...
	if(mysqli_num_rows($r_login)>0){
		...
		$res_login=mysqli_fetch_assoc($r_login);
